...
ปุ่ม "ลืมรหัสผ่าน?" ในหน้าเข้าสู่ระบบให้ผู้ใช้กรอกอีเมล แล้วกรอก **รหัสยืนยัน** ที่ได้รับ
...
จะเปิดเบราว์เซอร์ซึ่งส่ง session กลับเข้าแอปไม่ได้ ส่วนรหัสคือ recovery token
ตัวเดียวกันในรูปแบบที่คนพิมพ์เองได้

**ความยาวรหัสเป็นค่าตั้งของโปรเจค** ไม่ใช่ค่าคงที่ ดูและแก้ได้ที่ **Authentication →
Sign In / Providers → Email → Email OTP Length** (ตั้งได้ 6–10 หลัก) แอปรับได้ทั้งช่วงนี้
จึงไม่ต้องแก้โค้ดถ้าเปลี่ยนค่า
...
- ตั้ง **Minimum interval per user** ไว้ที่ `60` วินาที ให้ตรงกับปุ่ม "ส่งอีกครั้ง" ในแอป
  ที่นับถอยหลัง 60 วินาที ผู้ใช้จึงกดจนโดนปฏิเสธไม่ได้

### ⚠️ SMTP key หมดอายุเองเมื่อไม่ได้ใช้ 90 วัน

Brevo ระบุไว้ว่า *"SMTP keys also expire after 90 days of inactivity, regardless of the
set expiry date"* — แม้จะตั้งวันหมดอายุไว้ 1 ปี ถ้าไม่มีอีเมลส่งผ่าน key นี้เลยติดต่อกัน
90 วัน key จะตายเอง

เรื่องนี้สำคัญกับโปรเจคที่วางไว้นานระหว่างเทอม เพราะ **ปุ่มลืมรหัสผ่านจะพังเงียบ ๆ**
ไม่มีอะไรเตือน แอปยังขึ้นว่า "ส่งรหัสแล้ว" ตามปกติ แต่อีเมลไม่ออกจริง

ถ้าวันนำเสนอแล้วอีเมลไม่มา ให้ไล่ตามลำดับนี้:

1. **Brevo → Transactional → Logs** — ถ้าไม่มีรายการเลย แปลว่าอีเมลไม่เคยออกจาก Supabase
2. **Brevo → SMTP & API** — ดูคอลัมน์ Status ของ key ว่ายัง active อยู่ไหม
   ถ้าหมดอายุ สร้าง key ใหม่แล้วเอาไปแก้ช่อง Password ใน Supabase SMTP Settings
   (ช่องอื่นไม่ต้องแตะ)
3. **โฟลเดอร์ Spam** ของอีเมลปลายทาง — ส่งจากโดเมน gmail.com ที่ไม่ได้ตั้ง DKIM/DMARC
   จึงมีโอกาสเข้า Spam มากกว่าปกติ
4. **Supabase → Authentication → Rate Limits** — ว่าโดนจำกัดความถี่อยู่หรือเปล่า

### ค่าที่ตั้งไว้จริงในโปรเจคนี้

| ช่อง | ค่า |
|---|---|
| ผู้ให้บริการ | Brevo (แผนฟรี 300 ฉบับ/วัน) |
| Host | `smtp-relay.brevo.com` |
| Port | `587` |
| Sender email | อีเมลที่ยืนยันไว้ในหน้า Senders ของ Brevo |
| Minimum interval per user | `60` วินาที |

Username กับ SMTP key ไม่ได้บันทึกไว้ในรีโปโดยตั้งใจ — ใครได้ key ไปก็ส่งอีเมลในนาม
โปรเจคนี้ได้ ดูค่าจริงได้จาก Supabase Dashboard เท่านั้น และ**อย่าใส่ลงเล่มโปรเจคหรือ
สไลด์นำเสนอ**
